...
`scripts/verify-development-glb.mjs` parses the actual GLB with `GLTFLoader`, checks the real `SkinnedMesh`, bones and skin weights, requires all three clips and skeleton-targeted tracks, runs the Walk clip through `AnimationMixer`, and checks that the `LeftThigh` bone rotation changes. `tests/phase1.mjs` covers idle/walk/run selection, delta-time equivalence, normalized diagonals, smooth shortest-path turning, grounded/clamped motion, fixed-clock catch-up, asset contract shape, camera offset math, input reset on blur and finite repeated turns. Existing Phase 0/legacy tests remain unchanged.

Browser QA now uses a development-only query harness that dispatches timed `keydown`/`keyup` events through the real `KeyboardInput` listeners. It records numeric positions while keys are held and after release for W/S/A/D, all diagonals and Shift, plus animation weights during cross-fades. It also covers W-D-S-A turning, follow camera, orbit drag, focus loss, reload, navigation, invalid assets, legacy entry and three FPS samples. Machine-readable results are in [browser-qa-final.json](evidence/browser-qa-final.json).

## Limits

This is a foundation scene, not a wrestling game. It has no combat, opponent, ring, ropes, physics package, final character models, facial rig, backstage, entrance, story, or career behavior. The local sanity sample is approximately 60 FPS. The current repository baseline has 30 pre-existing lint diagnostics; Phase 1 files add zero. Phase 1 is QUALIFIED because the available in-app browser has no viewport-resize control, so that acceptance item remains unverified; `window.resizeTo` is a no-op at its fixed 1280×720 viewport.
