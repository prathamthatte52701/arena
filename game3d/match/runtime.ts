...
export class MatchRuntime {
  ...
  constructor(private readonly canvas: HTMLCanvasElement, private readonly callbacks: MatchCallbacks) {
    ...
    canvas.addEventListener('webglcontextlost', this.contextLost); this.timeout = setTimeout(() => { this.abort.abort(); this.callbacks.failure('Match characters timed out while loading.'); }, 15000);
  ...
  private frame = (now: number) => {
    ...
    if (this.playerCharacter && this.cpuCharacter) {
      this.clock.advance(dt, step => { const result = tickMatch(this.player, this.cpu, this.input.intent, step, this.elapsed); this.separation = result.separation; });
