...
import { CharacterPortraitRig } from './character/CharacterPortraitRig';
import { RheaFaceReview } from './dev/RheaFaceReview';
import { SAMPLE_LIP_SYNC_PHRASE, usePromoSpeech } from './performance/usePromoSpeech';
import type { BodyPoseName, Framing } from './body/types';
import type { CameraPerformanceSignal, CameraStateName } from './camera/types';
import type { GestureName } from './gestures/types';
import type { FaceControls, Tone } from './performance/types';
import { createSceneController } from './scenes/controller';
import { createSceneRuntimePlan, remapSceneRuntimePlan, restartSceneRuntimePlan, stopSceneRuntimePlan, type SceneRuntimePlan } from './scenes/orchestration';
import type { SceneName } from './scenes/types';
import type { CharacterId } from './characters/types.ts';
import { CHARACTER_REGISTRY, enabledCharacters, resolveCharacterId } from './characters/registry.ts';
import styles from '../app/promo-rhea/promo.module.css';

export function PromoExperience({ initialCharacterId = 'RHEA' }: { initialCharacterId?: unknown }) {
  const [characterId, setCharacterId] = useState<CharacterId>(() => resolveCharacterId(initialCharacterId));
  ...
  const [scenePlan, setScenePlan] = useState<SceneRuntimePlan>(() => createSceneRuntimePlan(character.scenes, { scene: 'INTERVIEW', text: '', tone: 'AUTO', mode: 'REST' }));
  ...
  const sceneController = useMemo(() => createSceneController(character.scenes), [character]);
  ...
  const selectScene = (name: SceneName) => {
    ...
    if (speaking) {
      const remapped = remapSceneRuntimePlan(character.scenes, createSceneRuntimePlan(character.scenes, { ...active, mode: 'SPEAKING' }), name);
    ...
    applyScenePlan(createSceneRuntimePlan(character.scenes, { scene: name, text: speech.text, tone: speech.tone, mode: 'REST' }));
  };

  const deliverPromo = () => {
    if (!speech.text.trim()) {
      activePresentation.current = null;
      applyScenePlan(createSceneRuntimePlan(character.scenes, { scene, text: speech.text, tone: speech.tone, mode: 'REST' }));
      speech.deliver();
      return;
    }
    const memory = { scene, text: speech.text, tone: speech.tone };
    const plan = createSceneRuntimePlan(character.scenes, { ...memory, mode: 'SPEAKING' });
  ...
  const stopPromo = () => {
    speech.stop();
    activePresentation.current = null;
    applyScenePlan(stopSceneRuntimePlan(character.scenes, scenePlan));
  };

  const replayPromo = () => {
    const memory = replayPresentation.current;
    if (!memory) return;
    const plan = restartSceneRuntimePlan(character.scenes, createSceneRuntimePlan(character.scenes, { ...memory, mode: 'SPEAKING' }));
    activePresentation.current = memory;
  ...
  const previewPhrase = () => {
    ...
    activePresentation.current = memory;
    applyScenePlan(createSceneRuntimePlan(character.scenes, { ...memory, mode: 'SPEAKING' }));
  ...
  return <main className={styles.shell} data-scene={sceneFrame.name} data-scene-container={sceneFrame.container} style={sceneStyle}>
    <header className={styles.header}>
      ...
      <label className={styles.characterSelector}>CHARACTER <select aria-label="Character" value={characterId} onChange={event => setCharacterId(resolveCharacterId(event.target.value))}>
        {enabledCharacters().map(option => <option key={option.id} value={option.id}>{option.name}</option>)}
      </select></label>
    ...
    <section className={styles.stage}>
      <div className={styles.set} data-scene-background={sceneFrame.background}>
        <CharacterPortraitRig character={character} controls={controls} framing={framing} bodyPose={bodyPose} gesturePreview={gesturePreview} gestureProgress={gestureProgress} cameraPreview={cameraPreview} cameraProgress={cameraProgress} cameraFinalPerformance={completedPerformance} scenePlan={scenePlan} sampleMouth={speech.sampleMouth} samplePerformance={speech.samplePerformance} />
        ...
        <div className={styles.nameplate}><p>{sceneFrame.label}</p><h1>{character.name}</h1><span>{controls.expression} / {controls.gaze}</span></div>
      </div>
      <aside className={styles.side} data-safe-text-area={sceneFrame.safeTextArea.placement}>
        {review ? <RheaFaceReview scene={scene} sceneConfig={sceneFrame} onSceneChange={selectScene} controls={controls} setControls={setControls} framing={framing} setFraming={setFraming} bodyPose={bodyPose} setBodyPose={setBodyPose} gesturePreview={gesturePreview} setGesturePreview={setGesturePreview} gestureProgress={gestureProgress} setGestureProgress={setGestureProgress} cameraPreview={cameraPreview} setCameraPreview={setCameraPreview} cameraProgress={cameraProgress} setCameraProgress={setCameraProgress} onStopSpeech={stopPromo} onSamplePhrase={previewPhrase} debugEnabled={speech.debugEnabled} setDebugEnabled={speech.setDebugEnabled} debug={speech.debug} /> :
          <div className={styles.prompt}><p className={styles.kicker}>{character.name} / {sceneFrame.label}</p><h2>{sceneFrame.prompt}</h2><p className={styles.question}>“What&apos;s next for you?”</p><p className={styles.description}>{sceneFrame.description}</p></div>}
