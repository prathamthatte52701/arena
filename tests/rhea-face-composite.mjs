...
const [rig, wrapper, styles, profile] = await Promise.all([
...
assert.equal((wrapper.match(/<canvas\b/g) ?? []).length, 0, 'the Rhea wrapper must not duplicate the canvas');
assert.equal((wrapper.match(/<CharacterPortraitRig\b/g) ?? []).length, 1, 'the Rhea wrapper must mount the generic rig once');
...
assert.match(rig, /width=\{character\.portrait\.width\}\s+height=\{character\.portrait\.height\}/, 'the portrait canvas must use the selected canonical profile dimensions');
